...
function concatenateStrings(value1, value2) {
  const temp = '';
  // console.log(temp.concat(value1, value2));
  return temp.concat(value1, value2);
}
...
function removeLastOccurrences(str, value) {
  // длина искомого значения
  const len = value.length;
  // находим идекс последнего входждения
  const lastIn = str.lastIndexOf(value);
  // console.log(lastIn + len);
  // console.log(lastIn + len, str.length);
  const firstPart = str.slice(0, lastIn);
  const secondPart = str.slice(lastIn + len, str.length);
  if (lastIn < 0) {
    return str;
  }
  if (lastIn + len === str.length) {
    return firstPart;
  }
...
function invertCase(str) {
  // логика -  перебираем каждую букву сравнивая
  // будет ли он равна заглавной букве, если равна то делаем её маленькой иначе большую.
  // добавляем букву в новую строку
  // возращаем строку
  let newStr = '';
...
// логика, rot 13 сдвигает на 13 букв изначальный текст.
// можно создать 2 констаты, и проверяя индекс каждой буквы, менять на такую же из второго индекса. Хорошим решением перебирать через map()
